...
$staging = Join-Path $env:TEMP "cardtx-report-update"
...
$body = Join-Path $env:TEMP "cardtx-update-body.json"
...
for ($i = 0; $i -lt 30; $i++) {
    ...
    $out = Join-Path $env:TEMP "cardtx-getdef.json"
    & az rest --method post --resource $FABRIC `
